...
2. 找它的交叉引用。AArch64 下字符串由 `adrp xN, page` + `add xN, xN, #lo12` 两条指令加载，扫描该指令对即可定位引用点。(可能版本原因编译出来的东西略有不同，这一步可以跳过)
3. 引用点落在「同步总闸函数」内。该函数读配置后决定：`true` 进入同步状态机，`false` 打印 `ResourceSync disabled` 并跳过。
4. 把函数入口改为 `mov w0, #0` + `ret`（补丁 A）；再把函数内紧跟配置读取的 `cbz w0` 改为 `nop`（补丁 B，双保险）。(与第二点同理)
